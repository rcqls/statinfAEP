...
(function($opal) {
  ...
  $opal.add_stubs(['$attr_accessor', '$new', '$[]', '$addChild', '$<<', '$shape', '$plot=', '$graph=', '$add', '$>=', '$each', '$call', '$method', '$to_Y', '$+', '$*', '$-', '$<', '$[]=', '$empty?', '$xylim', '$zoom', '$syncedChildren', '$synced?', '$select', '$include?', '$min', '$map', '$==', '$max', '$/', '$update', '$===', '$id', '$each_key', '$showZoom', '$>', '$graph', '$is_a?', '$tooltipContent', '$to_s', '$length', '$initStep', '$seq', '$dim', '$to_X', '$mean', '$maxPdf', '$stdDev', '$sample', '$pdf', '$type', '$map!', '$step', '$y', '$to_f', '$abs', '$set', '$initDistrib', '$setAsTransfOf', '$regular?', '$bounds', '$initXYLim', '$adjust', '$drawCont', '$drawDisc', '$<=', '$include', '$setParamsFrame', '$to_x', '$updateStatTestDistrib', '$draw', '$drawMean', '$drawSD', '$playCallables', '$nil?', '$paramsFrame', '$setDistrib', '$initTooltip', '$setAlphaFromQuantile', '$cdf', '$distrib', '$setAlpha', '$getSides', '$each_with_index', '$quantile', '$typeStatTest', '$drawAreaSide', '$setStyles', '$style', '$setPlot', '$alpha=', '$style=', '$initEvents', '$addCallable', '$attachShapes', '$setStatMode', '$reset', '$paramsFrame=', '$typeStatTest=', '$setPval', '$meanStyle=', '$sdStyle=', '$getContext', '$active=', '$updateVisible', '$join', '$power', '$p', '$inject', '$qbounds', '$to_a', '$prepare', '$quantize', '$keys', '$sort']);
...
if (k == null) k = nil;
    ...
    (function($base, $super) {
      ...
      def.$setPlot = function(plot) {
        var self = this;
        self.plot = plot;
        return self.graph = self.plot.$graph();
      };

      return (def.$initTooltip = function(shape) {
        var $a, $b, TMP_13, self = this;
        if (shape == null) {
          shape = self.shape
        }
        if (($a = shape['$is_a?']($scope.Array)) === false || $a === nil) {
          shape = [shape]};
        return ($a = ($b = shape).$each, $a._p = (TMP_13 = function(sh){var self = TMP_13._s || this;if (sh == null) sh = nil;
        
					sh.on("rollover",function(evt) {
						//console.log("mouseover!!!"+evt.stageX/cqls.m.stage.scaleX+":"+evt.stageY/cqls.m.stage.scaleY);
						cqls.m.tooltip.text=self.$tooltipContent(sh);
...
if (s == null) s = nil;if (i == null) i = nil;
      ...
      return (def.$tooltipContent = function(shape) {
        var self = this;
...
if (side == null) side = nil;if (i == null) i = nil;
      ...
      return (def.$tooltipContent = function(shape) {
        ...
        return self.context['$[]']("deltaPval");}else {return self.alpha}})();
        area = alpha;
        if (($a = ($b = self.statTest['$=='](statTestQuantile), ($b === nil || $b === false))) !== false && $a !== nil) {
          param = (function() {$case = self.statTest.$typeStatTest();if ("dp0"['$===']($case) || "dm0"['$===']($case) || "dp1"['$===']($case) || "dm1"['$===']($case)) {return self.statTest.$paramsFrame()['$[]'](0).$paramsFrame()['$[]'](1)}else {return self.statTest.$paramsFrame()['$[]'](1)}})();
          side = (function() {if (self.side['$==']("context")) {
            return self.context['$[]']("side")
            } else {
