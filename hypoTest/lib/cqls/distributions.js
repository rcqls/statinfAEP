...
function PowerDistribution(d0, n0){
	...
	if (d.type() === CONT ) {
		...
		values=d.values().map(function(x) {return Opal.CqlsHypo.$quantize(Math.pow(x,n));}).sort(ascend);
		//console.log(["values:",values]);
		for(var i=0; i<values.length;i++) pdf[i]=0;
		var tmp=d.values();
		for(i=0; i<tmp.length;i++) pdf[values.indexOf(Opal.CqlsHypo.$quantize(Math.pow(tmp[i],n)))]+=d.density(tmp[i]);
	...
	if (this.type() === DISC) {
		this.index = function(x) {
			return this.values().indexOf(Opal.CqlsHypo.$quantize(x));
		}
...
function Convolution2(d1, d2, b1, b2) {
	...
	var conv=Opal.CqlsHypo.Convolution.$new(d1,d2,b1,b2);
	//console.log(conv);
